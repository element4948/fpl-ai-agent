...
function DraftCard({ draft, lang }: { draft: Any; lang: 'mn' | 'en' }) {
  const t = dict[lang];
  return <details open={draft.mode === 'Best'}>
    <summary>{draftModeLabel(draft.mode, lang)} · £{draft.validation.totalCost.toFixed(1)}m · {draft.validation.valid ? t.allGood : t.check}</summary>
    <div className="tabs" style={{ marginTop: 12 }}>
      {draftExplanation(draft.mode, lang).map((x: string) => <span key={x} className="tab">{x}</span>)}
    </div>
    <div className="draft-grid">
      {draft.players?.map((p: ModelPlayer) => <div className="player-chip" key={p.id}><b>{p.name}</b><div className="row-meta">{p.position} · {p.team} · £{p.price}m</div></div>)}
    </div>
  </details>;
}
...
export default function Home() {
  ...
  useEffect(() => { fetch('/api/bootstrap').then(r => r.json()).then(setBoot).catch(e => setBoot({ error: e.message })); }, []);
  useEffect(() => { runDecision(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, []);
  ...
  async function runAnalyze() {
    setLoading(true);
    setAnalysis(null);
    const res = await fetch('/api/analyze', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ entryId: settings.entryId, freeTransfers: 1 }) });
  ...
  async function runLeague() {
    setLoading(true);
    setLeague(null);
    const res = await fetch('/api/league', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ entryId: settings.entryId, leagueId: settings.leagueId }) });
    ...
    <main id="top">
      <section className="hero">
        <div className="hero-panel">
          ...
          <div className="actions">
            <a className="button-link" href="#drafts">{t.navDrafts}</a>
            <a className="button-link secondary" href="#settings">{t.navSettings}</a>
          </div>
          <div className="pill-row"><span className="pill">{t.noHitDefault}</span><span className="pill">{t.budgetGuard}</span><span className="pill">{t.riskScore}</span><span className="pill">{t.chipHoldLogic}</span></div>
        </div>
        <Card title={t.seasonStatus} subtitle={t.optionalIds}>
          ...
          {!settings.entryId && <div className="notice" style={{ marginTop: 14 }}>{t.noId} {t.addLater}</div>}
      ...
      <Card title={t.thisWeekDecision} subtitle={t.decisionSub}>
        ...
        {decision?.transfer ? <div className="transfer-card" style={{ marginTop: 14 }}>
          ...
          <div className="tabs" style={{ marginTop: 10 }}>{decision.transfer.reasons.map((r: string) => <span className="tab" key={r}>{transferReason(r, lang)}</span>)}</div>
        </div> : null}
        <div className="actions" style={{ marginTop: 16 }}><button className="btn" disabled={loading} onClick={runDecision}>{loading ? t.loading : t.runDecision}</button></div>
      </Card>

      <section className="grid grid-3">
        <Card title={t.dataFoundation} subtitle={t.dataText}><Metric label={t.playersLoaded} value={boot?.playerCount ?? '...'} /><p className="muted">{t.officialApi}</p></Card>
        <Card title={t.ruleEngine} subtitle={t.ruleText}><Metric label={t.budget} value="£100.0m" /><Metric label={t.clubLimit} value="Max 3" /></Card>
        <Card title={t.riskEngine} subtitle={t.riskText}><Metric label={t.output} value={`${t.confidence} + ${t.risk}`} tone="good" /></Card>
      </section>

      <Card id="settings" title={t.settings} subtitle={t.optionalIds}>
        <div className="grid grid-3">
          <label className="field"><span>{t.entryId}</span><input value={settings.entryId || ''} placeholder={t.addLaterPlaceholder} onChange={e => updateSettings({ entryId: e.target.value })} /></label>
          <label className="field"><span>{t.leagueId}</span><input value={settings.leagueId || ''} placeholder={t.addLaterPlaceholder} onChange={e => updateSettings({ leagueId: e.target.value })} /></label>
          <label className="field"><span>{t.language}</span><select value={settings.lang} onChange={e => updateSettings({ lang: e.target.value as any })}><option value="mn">Монгол</option><option value="en">English</option></select></label>
          <label className="field"><span>{t.riskProfile}</span><select value={settings.riskProfile} onChange={e => updateSettings({ riskProfile: e.target.value as any })}><option value="safe">{t.safe}</option><option value="balanced">{t.balanced}</option><option value="aggressive">{t.aggressive}</option></select></label>
          <label className="field"><span>{t.goal}</span><select value={settings.goal} onChange={e => updateSettings({ goal: e.target.value as any })}><option value="overall">{t.overall}</option><option value="league">{t.league}</option><option value="both">{t.both}</option></select></label>
          <div className="field"><span>&nbsp;</span><button className="btn" onClick={persist}>{saved ? t.saved : t.save}</button></div>
        </div>
      </Card>

      <section id="team" className="grid grid-2">
        <Card title={t.navTeam} subtitle={t.liveTeamSub}>
          <button className="btn" disabled={loading} onClick={runAnalyze}>{loading ? t.loading : t.runTeamAnalysis}</button>
          {analysis?.summary && <div className="grid grid-4" style={{ marginTop: 16 }}>
          ...
          {analysis?.validation && <div className="notice" style={{ marginTop: 14 }}>{t.squadRules}: {analysis.validation.valid ? t.valid : t.invalid}</div>}
        ...
        <Card title={t.captainModel} subtitle={t.captainSub}>
          {captain?.length ? captain.slice(0, 6).map((p: Any, i: number) => <PlayerRow key={p.id} p={p} index={i + 1} lang={lang} />) : <p className="muted">{t.noData}</p>}
        </Card>
      </section>

      <section className="grid grid-2">
        <Card title={t.transferEngine} subtitle={t.transferSub}>
          {transfers.length ? transfers.map((x: Any, i: number) => <div className="transfer-card" key={i} style={{ marginBottom: 12 }}><div className="row-title">{x.out} → {x.in}</div><div className="row-meta">{t.expected}: +{x.expectedGain} · {t.cost}: {x.costChange}m · {t.hit}: {x.hitCost}</div><div className="tabs" style={{ marginTop: 10 }}>{x.reasons.map((r: string) => <span className="tab" key={r}>{transferReason(r, lang)}</span>)}</div></div>) : <p className="muted">{t.noSafeTransfer}</p>}
        </Card>
        <Card title={t.chipPlanner} subtitle={t.chipSub}>
          <div className="grid grid-2">{chips.map((c: Any) => <div className="chip-card" key={c.chip}><span className="badge green">{chipAction(c.action, lang)}</span><h3>{c.chip}</h3><p className="muted">{chipReason(c.chip, isPreSeason, lang)}</p><Metric label={t.confidence} value={`${c.confidence}%`} /></div>)}</div>
        </Card>
      </section>

      <Card id="league" title={t.leagueIntelligence} subtitle={t.leagueSub}>
        <button className="btn secondary" disabled={loading} onClick={runLeague}>{loading ? t.loading : t.runLeagueAnalysis}</button>
        {league?.message && <p className="muted">{t.noIdLeague}</p>}
        {league?.error && <p className="bad">{league.error}</p>}
        {league?.strategy && <div className="grid grid-3" style={{ marginTop: 16 }}><Metric label={t.strategy} value={strategyLabel(league.strategy, lang)} /><Metric label={t.managersAbove} value={league.managersAbove?.length || 0} /><Metric label={t.gap} value={league.pointsBehindLeader ?? '—'} /></div>}
        {league?.managersAbove?.length ? <div className="grid grid-2" style={{ marginTop: 16 }}>{league.managersAbove.slice(0, 12).map((m: Any) => <div className="manager-card" key={m.entry}><div className="rank">{m.rank}</div><div><b>{m.teamName}</b><div className="row-meta">{m.playerName} · {m.total} {t.points}</div></div><div className="score">+{m.gap}</div></div>)}</div> : null}
      </Card>

      <section id="drafts" className="grid grid-2">
        <Card title={t.topTargets} subtitle={t.topTargetsSub}>
          {top.length ? top.map((p: ModelPlayer, i: number) => <PlayerRow p={p} index={i + 1} key={p.id} lang={lang} />) : <div className="skeleton" />}
        </Card>
        <Card title={t.positionTargets} subtitle={t.positionTargetsSub}>
          <div className="split">
            {['GKP','DEF','MID','FWD'].map(pos => <div key={pos}><h3>{pos}</h3>{(topTargets[pos] || []).slice(0, 4).map((p: ModelPlayer) => <div className="player-chip" key={p.id} style={{ marginBottom: 8 }}><b>{p.name}</b><div className="row-meta">{p.team} · £{p.price}m · {t.risk} {p.risk}</div></div>)}</div>)}
          </div>
        </Card>
      </section>

      <Card title={t.draftTeams} subtitle={t.draftTeamsSub}>
        {boot?.drafts?.length ? boot.drafts.map((d: Any) => <DraftCard draft={d} key={d.mode} lang={lang} />) : <div className="skeleton" />}
      </Card>
    </main>
  </>;
}
