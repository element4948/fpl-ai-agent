...
- The urgent monitor runs every 30 minutes through `.github/workflows/urgent-monitor.yml`, avoiding Vercel Hobby's frequent-cron deployment restriction. Configure GitHub Actions repository secrets `PRODUCTION_URL` (the stable production origin) and `CRON_SECRET`. It sends only unseen high-severity My Team changes and decision reminders in the 24-hour, 6-hour, and 90-minute deadline windows; transfer-target warnings remain a separately labelled digest section.
...
- The decision digest leads with one action block, then separates My Team risks from transfer-target warnings. It includes the owner's team/rank when public, captain and vice, transfer/hold advice, a differential candidate, chip advice, price signals, league context, and a concise data-coverage line. When public picks are unavailable it sends a labelled watchlist and never presents a watchlist player as the owner's captain. Transfer advice states that one free transfer was assumed because the public API does not expose the owner's exact stored-transfer count. Manual refresh sends one current digest rather than repeating the previous digest first.
