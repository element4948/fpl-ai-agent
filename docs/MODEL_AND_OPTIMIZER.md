...
- Insufficient evidence never changes an otherwise legal squad into an illegal one; the UI combines `validation.valid` and `trust.status` only when enabling a final recommendation.
